    ...
    final displayed =
        filter.isEmpty
            ...
            : assets
                .where(
                  (a) =>
                      a['id']?.toLowerCase().contains(filter.toLowerCase()) == true ||
                      a['category']?.toLowerCase().contains(filter.toLowerCase()) == true,
                )
                .toList();

    return Scaffold(
      body: assets.isEmpty
          ? const Center(child: Text('No data found', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))
          : Padding(
